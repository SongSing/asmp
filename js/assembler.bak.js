...
class Assembler {
    ...
    assemble(str) {
        ...
        this.i8080.lengthArray.length = 0;
    ...
    resolveExpression16(e, addr) {
        ...
        for (var i = 0; i < e.length; i++) {
            let len = 0;
            let op = "";
            if (e.substr(i, len = 4) === (op = " OR ")  || e.substr(i, len = 5) === (op = " XOR ")) {
                for (var j = i + 1; j < e.length; j++) {
                    if (e.substr(j, 4) === " OR " || e.substr(j, 5) === " XOR " || (j === e.length - 1 && ++j)) {
                        let lhs = this.resolveExpression16(e.substr(0, i), addr);
                        if (lhs.error) return { error: lhs.error };
                        lhs = lhs.value;

                        let rhs = this.resolveExpression16(e.substr(i + len, (j - (i + len)) + 1), addr);
                        ...
                        e = lhs.value.toString() + e.substr(j + 1);
                        i = lhs.value.toString().length - 1;
                        break;
                    }
                }
            }
        ...
        for (var i = 0; i < e.length; i++) {
            ...
            if (e.substr(i, 5) === " AND ") {
                for (var j = i + 1; j < e.length; j++) {
                    if (e.substr(j, 5) === " AND " || (j === e.length - 1 && ++j)) {
                        let lhs = this.resolveExpression16(e.substr(0, i), addr);
                        if (lhs.error) return { error: lhs.error };
                        lhs = lhs.value;

                        let rhs = this.resolveExpression16(e.substr(i + 5, (j - (i + 5)) + 1), addr);
                        if (rhs.error) return { error: rhs.error };
                        rhs = rhs.value;
        ...
        for (var i = 0; i < e.length; i++) {
            let len = 0;
            let op = "";
            if (e.substr(i, len = 5) === " NOT " || (i === 0 && e.substr(i, len = 4) === "NOT ")) {
                for (var j = i + 1; j < e.length; j++) {
                    if (e.substr(j, 5) === " NOT " || (j === e.length - 1 && ++j)) {
                        let lhs = e.substr(0, i);
        ...
        for (var i = 0; i < e.length; i++) {
            let len = 0;
            let op = "";
            if (e.substr(i, len = 1) === (op = "+")  || e.substr(i, len = 1) === (op = "-")) {
                for (var j = i + 1; j < e.length; j++) {
                    if (e.substr(j, 1) === "+" || e.substr(j, 1) === "-" || ((j === e.length - 1 && ++j) && ++j)) {
                        let lhs = e.substr(0, i);
                        if (lhs.trim() === "") {
                            lhs = "0";
                        }
                        lhs = this.resolveExpression16(lhs, addr);
                        if (lhs.error) return { error: lhs.error };
                        lhs = lhs.value;

                        let rhs = e.substr(i + len, j - i - 1);
                        if (rhs.trim() === "") {
                            rhs = "0";
                        }
                        rhs = this.resolveExpression16(rhs, addr);
                        if (rhs.error) return { error: rhs.error };
                        rhs = rhs.value;
                        ...
                        e = lhs.value.toString() + e.substr(j);
                        i = lhs.value.toString().length - 1;
                        break;
                    }
                }
        ...
        for (var i = 0; i < e.length; i++) {
            ...
            let op = "";
            if (e.substr(i, len = 1) === (op = "*") || e.substr(i, len = 1) === (op = "/") 
             || e.substr(i, len = 5) === (op = " MOD ")  || e.substr(i, len = 5) === (op = " SHL ") || e.substr(i, len = 5) === (op = " SHR ")) {
                for (var j = i + 1; j < e.length; j++) {
                    if (e.substr(j, 1) === "*" || e.substr(j, 1) === "/"
                     || e.substr(j, 5) === " MOD " || e.substr(j, 5) === " SHL " || e.substr(j, 5) === " SHR " || (j === e.length - 1 && ++j)) {
                        let lhs = this.resolveExpression16(e.substr(0, i), addr);
                        if (lhs.error) return { error: lhs.error };
                        lhs = lhs.value;
                        ...
                        switch (op) {
                            case "*": lhs.value *= rhs.value; break;
                            case "/": lhs.value /= rhs.value; break;
                            case " MOD ": lhs.value %= rhs.value; break;
                            case " SHL ": lhs.value <<= rhs.value; break;
                            case " SHR ": lhs.value >>= rhs.value; break;
                        }
        ...
        if (e[e.length - 1] === "D") {
            ...
            for (var i = 0; i < e.length; i++) {
                ...
                if (isNaN(p)) {
                    return {
                        error: "invalid decimal numeral: " + og
                    };
                }
            }
